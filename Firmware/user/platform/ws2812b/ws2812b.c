...
void led_control(uint32_t color)
{
    ...
    uint32_t green = 0;
    uint32_t blue = 0;
    led_strip_hsv2rgb(color, 70, 100, &red, &green, &blue);
    // for (uint8_t idx = 0; idx < NUM_OF_LED; idx++)
    // {
    //     strip->clear(strip, 100);
    // }
    for (uint8_t idx = 0; idx < NUM_OF_LED; idx++) {
        ESP_ERROR_CHECK(strip->set_pixel(strip, idx, red, green, blue));
    }
...
void led_init()
{
    rmt_config_t config = RMT_DEFAULT_CONFIG_TX(RMT_TX_GPIO, RMT_CHANNEL_0);
    config.clk_div = 2;

    ESP_ERROR_CHECK(rmt_config(&config));
    ESP_ERROR_CHECK(rmt_driver_install(config.channel, 0, 0));

    led_strip_config_t strip_config = LED_STRIP_DEFAULT_CONFIG(NUM_OF_LED, (led_strip_dev_t)config.channel);
